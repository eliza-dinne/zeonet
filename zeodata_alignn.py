...
def get_triplets(edge_index):
    ...
    edge_index_triplets = torch.stack([idx_ji, idx_kj], dim=0)
 
    return idx_i, idx_j, idx_k, idx_kj, idx_ji, edge_index_triplets
...
def create_graphs(zeo : str = 'TON', triplets : bool = False):
    ...
    if triplets:
        idx_i_1, idx_j_1, idx_k_1, idx_kj_1, idx_ji_1, edge_index_triplets = get_triplets(edge_index)
...
def unpack_batch(batch, device : str = 'cpu'):
    ...
    edge_index_triplets = batch.edge_index_triplets
    dist = batch.edge_attr.float()
    angle = batch.angle.float()
    y = batch.y.float()
    btch = batch.batch
